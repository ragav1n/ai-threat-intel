...
phase, so calibration can be re-analysed without re-querying the LLM:
    ...
    python scripts/run_calibration_study.py            # full run (uses cache if present)
    python scripts/run_calibration_study.py --fresh    # force re-running the LLM
"""
import json
import logging
import os
...
PRED_PATH = "data/evaluation/calibration_predictions.json"
OUT_PATH = "data/evaluation/calibration_study.json"
...
def collect_predictions() -> list:
    """Run extraction + LLM verification over the ground truth; label each IOC."""
